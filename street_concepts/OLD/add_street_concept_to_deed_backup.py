...
def write_to_files(deviates, dfs, **kwrgs):
    ...
    try: 
        ...
        # key_concepts = kwrgs['concept_street'] = set in for loop
        ...
        for streets_col, concepts_col in update_mapping.items():
            value_map = dfs[1].dropna(subset=[concepts_col]).set_index(kwrgs['concept_street'])[concepts_col].to_dict()
...
def merge_data(dfs, **kwrgs):
    ...
    try:
        ...
        merged = pd.merge(dfs, left_on= kwrgs['extr_str'], right_on='straat', how='left') ####
...
if __name__ == '__main__':
        ...
        df_street, dfs_out = output_to_file_and_db(dfs, dfs_out, deed, **kwrgs)