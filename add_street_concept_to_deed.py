...
def output_to_file_and_db(merged, df_streets, deed, **kwrgs):
    ...
    try:
        ...
        if response.status_code != 200:
            logging.info("...Try to read again...")
            time.sleep(3)
            response = api.get_record(uuid)
            if response.status_code != 200:
                logging.error(f"Reading failed for {uuid}")
        #elif response.status_code:
        #    print(response.status_code)
        else: 
            print('ín the else')
            api.update_record_type(df_streets, turtle)   # ECHIE ECHIE ECHIE acc omgeving
    except:
        logging.exception(f'Error while updating csv to memorix')
        raise
